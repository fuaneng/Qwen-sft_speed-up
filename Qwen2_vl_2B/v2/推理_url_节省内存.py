...
model = Qwen2VLForConditionalGeneration.from_pretrained(
    r"/DATA/LYC/Qwen2_VL_2B_SFT-main/models/Qwen2-VL-2B-Instruct",
    torch_dtype=torch.float16,  # 使用 float16 降低内存需求
    device_map="auto"  # 自动分配设备（GPU 或 CPU）
)
model = PeftModel.from_pretrained(model, model_id=r"/DATA/fyn/output_dir/models/checkpoint-9424", config=config)
processor = AutoProcessor.from_pretrained(r"/DATA/LYC/Qwen2_VL_2B_SFT-main/models/Qwen2-VL-2B-Instruct")
...
# 定义 Excel 文件路径和表名
excel_file_path = r"/DATA/fyn/tairn/gemini数据_合并后_doubao自扩展.xlsx"
sheet_name = "端到端可灵1"  # Excel 分表名称
output_excel_file_path = r"/DATA/fyn/tairn/推理结果_端到端_可灵_通义0729.xlsx"  # 输出结果文件路径
...
try:
    ...
    for col in required_columns:
        if col not in df.columns:
            ...
            exit()
    print(f"成功读取 Excel 文件 '{excel_file_path}' 的工作表 '{sheet_name}'，共 {len(df)} 行数据。")
            ...
            if processed_image is None: # 如果图片处理失败
                ...
                try:
                    ...
                    if "CUDA out of memory" in str(e):
                        print(f"CUDA 内存不足: {e}")
                        generated_text = f"推理失败: CUDA 内存不足"
                    else:
