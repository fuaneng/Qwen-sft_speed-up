...
SHEET_NAME = "豆包"
OUTPUT_EXCEL_FILE_PATH = r"/DATA/z60046962/Qwen2_5_SFT-main/train/data_tuili/推理结果_vllm_打榜2_豆包-8400.xlsx"

# 推理参数
BATCH_SIZE = 8  # vLLM 可以处理更大的批次，根据你的显存调整 (7B模型建议从4或8开始尝试)
SAVE_EVERY_N_BATCH = 2 # 每处理2个批次就保存一次，防止意外
...
def resize_image(image_bytes, max_dim=512):
    ...
    try:
        ...
        # 将缩放后的图片存回字节流
        buffered = io.BytesIO()
        img.save(buffered, format="PNG") # 使用PNG格式以支持透明度并减少伪影
...
if __name__ == "__main__":
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="处理数据中"):
        ...
        if image_url.startswith(("http://", "https://")):
            try:
                # 下载图片到内存
                response = requests.get(image_url, timeout=15) # 增加超时时间
